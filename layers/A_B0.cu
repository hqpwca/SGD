...
__global__ void pixel_forward_projection(double *proj, const double *vol, int3 n3xyz, double3 d3xyz, int nu, double3 src, double3 puv, double3 dtv) { // nz = 1, dz = 0, src.z = 0
...
__global__ void pixel_backward_projection(const double *proj, double *vol, int3 n3xyz, double3 d3xyz, int nu, double3 src, double3 puv, double3 dtv) { // nz = 1, dz = 0, src.z = 0
    //int ip = (blockIdx.x * blockDim.x) + threadIdx.x;
    int iu = (blockIdx.x * blockDim.x) + threadIdx.x;
...
void A_B0::project(Matrix &vol, MatrixD &proj, double weight)
{
}

void A_B0::back_project(Matrix &vol, MatrixD &proj, double weight)
{
}
