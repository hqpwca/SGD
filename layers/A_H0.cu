...
__global__ void h0_forward_projection(double *proj, const double *vol, const float *vecs, const float *pm, int3 n3xyz, float3 d3xyz, int nu, float3 src, float3 puv, float3 dtv, int z_size, int np, int ip) {
    ...
    for (int ti = 0; ti <= (maxu - minu); ++ti, sx += puv.x, sy += puv.y) {
        ...
        double conv = linear_convolution_1d(y, vblur, va, vb) + linear_convolution_1d(y, vblur, vb, vc) + linear_convolution_1d(y, vblur, vc, va);
...
__global__ void h0_backward_projection(const double *proj, double *vol, const float *vecs, const float *pm, int3 n3xyz, float3 d3xyz, int nu, float3 src, float3 puv, float3 dtv, int z_size) {
    int ic = (blockIdx.x * blockDim.x) + threadIdx.x;
    // int blocky = (blockIdx.y * blockDim.y) + threadIdx.y;
    int il = (blockIdx.y * blockDim.y) + threadIdx.y;
...
void A_H0::project(MatrixD &vol, MatrixD &proj, double weight)
{
    for(int p=0; p<geodata->np; p++){
        ...
        std::cout << p << std::endl;
#endif
        h0_forward_projection<<<vgrid, vblock>>>(proj(0), vol(0), nullptr, geodata->pmis(p*12), geodata->nxyz, 
...
void A_H0::back_project(MatrixD &vol, MatrixD &proj, double weight)
{
    for(int p=0; p<geodata->np; p++){
        ...
        h0_backward_projection<<<vgrid, vblock>>>(proj(int(p * geodata->nuv.x)), vol(0), nullptr, geodata->pmis(p*12), geodata->nxyz, 
                                                        make_float3(geodata->dxyz.x, geodata->dxyz.y, geodata->dxyz.z), geodata->nuv.x,
                                                        make_float3(*geodata->srcs[p*3], *geodata->srcs[p*3+1], *geodata->srcs[p*3+2]),
                                                        make_float3(*geodata->puvs[p*3], *geodata->puvs[p*3+1], *geodata->puvs[p*3+2]),
...
extern "C" {
    ...
    A_H0 *h0_init(int nl, int nc, int np, int nu, double dt, double du, double lsd, double lso, double *angles, double *dz, double *drho){
        ...
        A_H0 *h0_layer = new A_H0(geo);

        return h0_layer;
    }

    int h0_forward_projection(double *b, double *x, A_H0 *h0_layer) {
        int nc = h0_layer->geodata->nxyz.x;
        int nl = h0_layer->geodata->nxyz.y;

        int np = h0_layer->geodata->np;
        int nu = h0_layer->geodata->nuv.x;
    ...
    void h0_backward_projection(double *b, double *x, A_H0 *h0_layer) {
        ...
        h0_layer->back_project(vol, proj, 1.0);
