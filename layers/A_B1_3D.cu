...
// Fan Beam only
__global__ void b1_3d_forward_projection(double *proj, const double *vol, const float *vecs, const float *pm, int3 n3xyz, float3 d3xyz, int nu, float3 src, float3 puv, float3 dtv, int z_size, int np, int ip) {
...
__global__ void b1_3d_backward_projection(const double *proj, double *vol, const float *vecs, const float *pm, int3 n3xyz, float3 d3xyz, int nu, float3 src, float3 puv, float3 dtv, int z_size) {
    int ix = (blockIdx.x * blockDim.x) + threadIdx.x;
    int blocky = (blockIdx.y * blockDim.y) + threadIdx.y;
    // int iy = (blockIdx.y * blockDim.y) + threadIdx.y;
...
void A_B1_3D::project(MatrixD &vol, MatrixD &proj, double weight)
{
    for(int p=0; p<geodata->np; p++){
        ...
        std::cout << p << std::endl;
#endif
        b1_3d_forward_projection<<<vgrid_z, vblock>>>(proj(0), vol(0), (*vecs)(p * geodata->nuv.x * 2), geodata->pmis(p*12), geodata->nxyz, 
...
void A_B1_3D::back_project(MatrixD &vol, MatrixD &proj, double weight)
{
    for(int p=0; p<geodata->np; p++){
        ...
        b1_3d_backward_projection<<<vgrid, vblock>>>(proj(int(p * geodata->nuv.x)), vol(0), (*vecs)(p * geodata->nuv.x * 2), geodata->pmis(p*12), geodata->nxyz, 
                                                        make_float3(geodata->dxyz.x, geodata->dxyz.y, geodata->dxyz.z), geodata->nuv.x,
                                                        make_float3(*geodata->srcs[p*3], *geodata->srcs[p*3+1], *geodata->srcs[p*3+2]),
                                                        make_float3(*geodata->puvs[p*3], *geodata->puvs[p*3+1], *geodata->puvs[p*3+2]),
...
extern "C" {
    ...
    int b1_forward_projection(double *b, double *x, A_B1_3D *b1_layer) {
    ...
    void b1_backward_projection(double *b, double *x, A_B1_3D *b1_layer) {
        int nx = b1_layer->geodata->nxyz.x;
        int ny = b1_layer->geodata->nxyz.x;
